...
public class UpdateGroupInfoProcessor
        implements Processor<SendGroupCommandReq, SendGroupCommandResp> {

    private final Logger logger = LoggerFactory.getLogger(UpdateGroupInfoProcessor.class);
    private final String topicName;
    private final GenericProducer<String, MessagePayload> producer;
    private final ExecutorService executorService;
    private final RippleStorageFacade storageFacade;

    public UpdateGroupInfoProcessor(
            String topicName,
    ...
    public SendGroupCommandResp handle(SendGroupCommandReq request) throws Exception {
        List<Long> currentMembers = storageFacade.getGroupMemberIds(request.getGroupId());
